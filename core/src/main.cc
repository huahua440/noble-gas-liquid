...
struct Electron {
    ...
    std::vector<double> lastinteractionposition; // Position at last interaction
    double frequency; //
    double freemeantime; //
    double lambda; //
    ...
    Electron(double total_time, double volts, std::vector<double> starting_pos,
             ...
             std::vector<double> lastinteractionposition, double lambda,
        ...
        this->lastinteractionposition = lastinteractionposition;
...
void gas_update(Electron &thiselec, int &total_ionizations, double T,
    ...
    double xsec_ex_16 = x_sec_ex(norm(thiselec.v), 11.55, sigma_ex, eV_min, eV_max, eV_number);
    double xsec_ex_18 = x_sec_ex(norm(thiselec.v), 13, sigma_ex, eV_min, eV_max, eV_number);
    double xsec_ex_19 = x_sec_ex(norm(thiselec.v), 14, sigma_ex, eV_min, eV_max, eV_number);
    double xsec_tot = xsec_el + xsec_ioni + xsec_ex_16 + xsec_ex_18 + xsec_ex_19;
...
//	thiselec.freemeantime=1/thiselec.frequency;
    ...
    double ex_16_prob = probability( xsec_ex_16,xsec_tot,factor_null_collision);
    double ex_18_prob = probability( xsec_ex_18,xsec_tot,factor_null_collision);
    double ex_19_prob = probability( xsec_ex_19,xsec_tot,factor_null_collision);
    ...
    if (prob < ion_prob) {
       ionization(thiselec, total_ionizations,  volts, ionization_energy, generator);
        hasinteracted = true;
    } else if (prob < (ion_prob + ex_16_prob)) {
        excitation(thiselec, 11.55, m_e);
        hasinteracted = true;
    } else if (prob < (ion_prob + ex_16_prob + ex_18_prob)) {
        excitation(thiselec, 13, m_e);
        hasinteracted = true;
    } else if (prob < (ion_prob + ex_16_prob + ex_18_prob + ex_19_prob)) {
        excitation(thiselec, 14, m_e);
        hasinteracted = true;
    } else if (prob < (ion_prob + ex_16_prob + ex_18_prob + ex_19_prob + col_prob)) {
    ...
    if (hasinteracted) {
        thiselec.dist = norm({(thiselec.lastinteractionposition[0] - thiselec.position[0]),
                              (thiselec.lastinteractionposition[1] - thiselec.position[1]),
                              (thiselec.lastinteractionposition[2] - thiselec.position[2])});
        thiselec.lastinteractionposition = thiselec.position;
    }
    ...
    xsec_ex_16 = x_sec_ex(norm(thiselec.v), 11.55, sigma_ex, eV_min, eV_max, eV_number);
    xsec_ex_18 = x_sec_ex(norm(thiselec.v), 13, sigma_ex, eV_min, eV_max, eV_number);
    xsec_ex_19 = x_sec_ex(norm(thiselec.v), 14, sigma_ex, eV_min, eV_max, eV_number);
    xsec_tot = xsec_el + xsec_ioni + xsec_ex_16 + xsec_ex_18 + xsec_ex_19;
...
void liquid_update(Electron &thiselec, int &total_ionizations, double T,
    ...
    double xsec_ex_18 = x_sec_ex(norm(thiselec.v), 13, sigma_ex, eV_min, eV_max, eV_number);
    double xsec_ex_19 = x_sec_ex(norm(thiselec.v), 14, sigma_ex, eV_min, eV_max, eV_number);
    double xsec_tot = xsec_el_e + xsec_el_m + xsec_ioni + xsec_ex_16 + xsec_ex_18 + xsec_ex_19;
    ...
    if (prob < ion_prob) {
        ...
        excitation(thiselec, 14, m_e);
        hasinteracted = true;
    } else if (prob < (ion_prob + ex_16_prob + ex_18_prob + ex_19_prob + col_prob_e + col_prob_m)) {
    ...
    xsec_ex_18 = x_sec_ex(norm(thiselec.v), 13, sigma_ex, eV_min, eV_max, eV_number);
    xsec_ex_19 = x_sec_ex(norm(thiselec.v), 14, sigma_ex, eV_min, eV_max, eV_number);
    xsec_tot = xsec_el_e + xsec_el_m + xsec_ioni + xsec_ex_16 + xsec_ex_18 + xsec_ex_19;
...
void do_loop (int volts,double time_cut,int i,std::ofstream& fileID,int factor_null_collision,std::string atom_name,int state) //distance_cut
{
...
//                        }
    ...
    std::vector<double> lastinteractionposition = {0, 0, 0};
    ...
    electron_list.push_back(Electron(0, volts, pos, v, generator, uniform_field_ex, lastinteractionposition, lambda,frequency, freemeantime));
    ...
    const int write_every = 10000;

    while (electron_list[0].total_time < time_cut) { //electron_list[0].z<distance_cut
        ...
        if (simulation_step % write_every == 0) {
            ...
            vd = std::abs((x - starting_x) / t);
           // s = thiselec.scatteringangle;
            fileID << std::scientific;
            fileID << t * 1e9 << ", "  << x * 1e6 << ", " << y * 1e6 << ", " << z * 1e6 << ", "  << energy << ", " << vd  << "\n";
            //  fileID << t * 1e9 << ", " << dt * 1e9 << ", " << x * 1e6 << ", " << y * 1e6 << ", " << z * 1e6 << ", " << vx << ", " << vy << ", " << vz << ", " << energy << ", " << vd << ", "  << total_ionizations << "\n";
        }
        simulation_step++;
    }
...
*/
    ...
    std::vector<int> voltages = {1 ,  5,    10,  20,   50,    70,  100,  200,  500,  1000, 2000};
    std::vector<double> cutoffs= {8E-6,8E-6, 5E-6, 5E-6, 1E-6, 1E-6, 5E-7, 5E-7, 4E-7, 4E-7, 4E-7};
    int state=0;

    //if (world_rank == 0) {
    for (int v_index = 3; v_index < 4; v_index++)
    {
        int volts = voltages[v_index];
        double time_cut = cutoffs[v_index];
      //  double distance_cut = 1e-6;
       // std::cout << v_index << std::endl;

        //    int num_simulations = 20;
        //    int num_per_process = (num_simulations + world_size - 1) / world_size; // 确保均匀分配
        //std::cout <<"world_size  "<< world_size << std::endl;
        /////////////////////////////////////////////openmp
        //    int threads = 100; // 设置线程数，可以根据你的系统调整
        ...
        int n0 = 1;
        for (int j = n0; j < n0+THREAD_NUM - 1; j ++) {
            //     std::cout <<"i  "<< i << std::endl;
            threads.emplace_back([state,volts,time_cut,n0,j, AVG_NUM]()   //distance_cut
                                 {
                                     for (int i =n0+ (j-n0) * AVG_NUM; i < n0+(j-n0 + 1) * AVG_NUM; i++)
                                     {
//////////////////////////////////////////////////////
                                         int factor_null_collision=2;
                                         std::string atom_name="ar";
                                         std::string folderPath = "result/ar/";
                                         std::string filename;
                                         if(state==0){
                                         ...
                                         do_loop(volts,time_cut,i,fileID,factor_null_collision,atom_name,state); //distance_cut
                                         fileID.close();
                                     }
                                 });
        }
